...
extension CatalogStore {
    ...
    func upsertTakeoutArchive(_ archive: TakeoutArchive) throws {
        try journaled("takeout_archives", [archive.id.uuidString]) {
            try database.run("""
...
extension CatalogStore {
    ...
    func repointZipMembers(
        ...
        // Where a copy lives on a drive is a fact about the drive, not about
        // this device — so another device needs to be told when it moves, or it
        // will go looking in the old place.
        return affected
    }
}
